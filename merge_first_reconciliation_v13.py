# merge_first_reconciliation_v13.py
# ================================================================
...
# ================================================================
# STREAMLIT CLOUD EXCEL DEPENDENCY HANDLING
# ================================================================
# Do NOT import openpyxl at module import time. If Streamlit Community Cloud
# has not yet installed requirements.txt, a top-level import crashes the whole
# app before the UI can load. Excel readers below check the dependency only
# when an .xlsx file actually needs to be read.
...
def safe_read_excel(file_obj, **kwargs):
    """
    Read Excel with a deployment-friendly dependency check.

    Streamlit Community Cloud must install openpyxl from requirements.txt.
    If it is missing, raise a concise runtime error AFTER the app UI has loaded,
    ...
    try:
        ...
        raise RuntimeError(
            "Excel support is not installed on this Streamlit deployment. "
            "Your GitHub repository must contain a file named exactly "
            "'requirements.txt' either beside merge_first_reconciliation_v13.py "
            "or at the repository root. That file must contain "
            "'openpyxl==3.1.5'. Commit the file, then reboot/redeploy the app. "
            "Also remove/rename competing dependency files such as uv.lock, "
            "Pipfile, environment.yml, or another requirements file that may "
            "be taking precedence."
        ) from exc

    return pd.read_excel(
        excel_input(file_obj),
        engine="openpyxl",
        **kwargs,
    )
...
def clean_paymeter(file_obj):
    if suffix_of(file_obj) in {".xlsx", ".xls"}:
        df = safe_read_excel(file_obj, dtype=str)
...
def read_bank(file_obj):
    raw = safe_read_excel(file_obj, header=None, dtype=str)
    header_row = None
...
def main():
    ...
    st.set_page_config(
        page_title="Merge-First KYC Reconciliation V13",
        page_icon="🧾",
        layout="wide",
    )
    st.title("🧾 Merge-First KYC Reconciliation V13")
    st.caption(
...
workbook can be produced on a later run when required.
    ...
    try:
        ...
        st.download_button(
            ...
            file_name="Merge_First_KYC_Management_Report_V13.xlsx",
            mime=(
        ...
        if build_full_workbook:
            ...
            st.download_button(
                ...
                data=full_excel,
                file_name="Merge_First_KYC_Reconciliation_V13.xlsx",
                mime=(
                    "application/vnd.openxmlformats-officedocument."
                    "spreadsheetml.sheet"
                ),
